...
//a probability map representing the chances of a link failiure are produced.
{
  ...
  // Adds k additional edges to the graph
  {
    ...
    {
      if (network_graph[vertex_list[i]][vertex_list[j]] != true)
      //checks if there's an edge between two random vertices 
      {
        network_graph[vertex_list[i]][vertex_list[j]] = true;
        network_graph[vertex_list[j]][vertex_list[i]] = true;
        j = size;
  ...
  for (int i = 0; i < size; i++) 
  //initializes link_failiure_matrix
  {
    for (int j = 0; j < size; j++)
    {
      if (network_graph[i][j] == 0 || i == j) 
      //If there is no link between the two nodes, or if the node is adjacent to itself, the probability of failiure is 0
      {
        link_failiure_matrix[i][j] = 0;
      }
      else
      {
        link_failiure_matrix[i][j] =static_cast <float> (rand() % (RAND_MAX/10))/static_cast <float> (RAND_MAX); 
...
void network::resize_network(vector<int> deleted_nodes)
{
  ...
  // If there are fewer than 2 nodes in the network, continuing the simulation is pointless
  {
    for(int i = 0; i < deleted_nodes.size(); i++)
    {
      ...
      //Above erases the rows representing the deleted nodes

      size--;
      //There is now one less row in the matrix, meaning the number 
      //of indices in the column to be erased is reduced by one

      for (int j = 0; j < size; j++)
      {
        //Erases the columns representing the deleted nodes
      ...
      //Since the size has decreased, the next deleted node must be decreased by the same amount to accurately reflect the index
      {
        deleted_nodes[i + 1] -= i + 1;
      }
    }
  }
  else
